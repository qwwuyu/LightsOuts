...
public class ObservableColor {
    ...
    public void updateHueSat(float hue, float sat, ColorObserver sender, int action) {
        hsv[0] = hue;
        hsv[1] = sat;
        notifyOtherObservers(sender, action);
    }

    public void updateValue(float value, ColorObserver sender, int action) {
        hsv[2] = value;
        notifyOtherObservers(sender, action);
    }

    public void updateAlpha(int alpha, ColorObserver sender, int action) {
        this.alpha = alpha;
        notifyOtherObservers(sender, action);
    }
    ...
    public void updateColor(int color, ColorObserver sender) {
        Color.colorToHSV(color, hsv);
        alpha = Color.alpha(color);
        notifyOtherObservers(sender, -1);
    }

    private void notifyOtherObservers(ColorObserver sender, int action) {
        for (ColorObserver observer : observers) {
            if (observer != null && observer != sender) {
                observer.updateColor(this, action);
            }
        }
    }

    public interface ColorObserver {
        void updateColor(ObservableColor observableColor, int action);
    }
}